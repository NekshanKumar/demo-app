...
export default function Header() {
  ...
  const currentItem = menu.find(m => location.pathname.startsWith(m.path)) || menu[0];
  ...
  return (
    <header className="shadow-header py-[16px] px-[48px] flex justify-between items-center">
      ...
      <MobileDrawer open={drawerOpen} onClose={() => setDrawerOpen(false)} user={user} onLogout={() => { logout(); navigate("/login"); }} />
    </header>
  );
}
