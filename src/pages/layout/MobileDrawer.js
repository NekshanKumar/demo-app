    ...
    <Fragment>
      ...
      <aside className={`fixed top-0 left-0 z-50 h-full w-full max-w-[320px] bg-white shadow-2xl transform transition-transform ${open ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        <nav className="p-4 flex flex-col gap-2 h-[calc(100%-160px)] overflow-auto">
          <NavLink to="/dashboard" onClick={onClose} className={({ isActive }) => `px-4 py-3 rounded-lg text-sm font-medium ${isActive ? "bg-primary100 text-primary600" : "hover:bg-primary50 text-gray-700"}`}>Dashboard</NavLink>
          <NavLink to="/userProfile" onClick={onClose} className={({ isActive }) => `px-4 py-3 rounded-lg text-sm font-medium ${isActive ? "bg-primary100 text-primary600" : "hover:bg-primary50 text-gray-700"}`}>User Profile</NavLink>
